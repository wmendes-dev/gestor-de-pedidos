...
public class PedidoConfirmadoStrategy implements IPedidoSituacaoStrategy {
    ...
    public void atualizarSituacaoPedido(Pedido pedido) {
        if (!pedido.getSituacao().equals(SituacaoPedidoEnum.CRIADO)
            ...
            throw new RegraNegocioException("Não é possível atualizar a situação do pedido para '%s'"
                    .formatted(SituacaoPedidoEnum.CONFIRMADO.getDescricao()));
        }

        this.pedidoRepository.atualizarSituacaoPedido(pedido.getIdPedido(), SituacaoPedidoEnum.CONFIRMADO);
    }

    @Override
    public EventoAtualizacaoSituacaoPedidoEnum getEventoAtualizacaoSituacaoPedido() {
        return EventoAtualizacaoSituacaoPedidoEnum.ESTOQUE_MOVIMENTADO;
    }
