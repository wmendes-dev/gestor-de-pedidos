...
public class PedidoPagamentoErroReservaEstoqueStrategy implements IPedidoSituacaoStrategy {
    ...
    public void atualizarSituacaoPedido(Pedido pedido) {
        if (!pedido.getSituacao().equals(SituacaoPedidoEnum.CRIADO)) {
            throw new RegraNegocioException("Não é possível atualizar a situação do pedido para '%s'"
                    .formatted(SituacaoPedidoEnum.ERRO_RESERVA_ESTOQUE.getDescricao()));
        }

        this.pedidoRepository.atualizarSituacaoPedido(pedido.getIdPedido(), SituacaoPedidoEnum.ERRO_RESERVA_ESTOQUE);
    }

    @Override
    public EventoAtualizacaoSituacaoPedidoEnum getEventoAtualizacaoSituacaoPedido() {
        return EventoAtualizacaoSituacaoPedidoEnum.ERRO_RESERVA_ESTOQUE;
    }
